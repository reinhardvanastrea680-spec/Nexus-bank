...
function AdminTransactionsPage() {
  ...
  const filtered = transactions.filter((tx) => {
    // If filtering by user, only show that user's transactions
    if (viewMode === "user" && selectedUserId && tx.userId !== selectedUserId) return false;
    // Apply search term
    return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          ...
          style={{
            background: viewMode === "all" ? "linear-gradient(135deg,#38BDF8,#6366F1)" : "rgba(255,255,255,0.05)",
            color: viewMode === "all" ? "#fff" : "#8A9BB5",
            border: viewMode === "all" ? "none" : "1px solid rgba(255,255,255,0.08)",
          }}
        >
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl text-sm font-semibold transition-all"
          style={{
            background: viewMode === "user" ? "linear-gradient(135deg,#38BDF8,#6366F1)" : "rgba(255,255,255,0.05)",
            color: viewMode === "user" ? "#fff" : "#8A9BB5",
            border: viewMode === "user" ? "none" : "1px solid rgba(255,255,255,0.08)",
          }}
        >
          Filter by User
        </button>

        {/* User dropdown — only visible when Filter by User is active */}
        {viewMode === "user" && (
          <select
            ...
            style={{
              ...
              color: selectedUserId ? "#fff" : "#8A9BB5",
              minWidth: 220,
            }}
          >
